...
- `模型 API 中转`：稳定栏目，用于介绍并分发到对外售卖的模型 API 中转站。

## 内容优先级

第一梯队：

- `Blog`：技术类、知识类、学习类文章为主，也允许少量其他内容。内容源来自 `Tonkic/tonkic-obsidian-vault` 的 Markdown 笔记。
...
由于当前 New API 的 `OPTIONS` 预检响应有 CORS 头，但实际 `GET /api/status` 和 `GET /api/pricing` 响应没有 `Access-Control-Allow-Origin`，GitHub Pages 这类静态站不能在浏览器中直接读取接口。当前页面使用最近公开信息快照展示模型与价格，并提供中转站跳转。
...
- Blog 浏览体验采用“知识库目录树优先，时间流为辅”：目录树来自 Obsidian 的 `sourcePath`，由 Fumadocs 构建；最近同步列表只作为辅助入口。
